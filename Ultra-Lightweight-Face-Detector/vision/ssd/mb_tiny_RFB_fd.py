...
def create_Mb_Tiny_RFB_fd(num_classes, is_test=False, device="cuda"):
    ...
    # 5-point landmark head: mirrors regression_headers (same in_channels, same
    # conv type per level), out_channels = priors_per_cell * 10 -> [30, 20, 20, 30]
    landmark_headers = ModuleList([
        SeperableConv2d(in_channels=base_net.base_channel * 4, out_channels=3 * 10, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=base_net.base_channel * 8, out_channels=2 * 10, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=base_net.base_channel * 16, out_channels=2 * 10, kernel_size=3, padding=1),
        Conv2d(in_channels=base_net.base_channel * 16, out_channels=3 * 10, kernel_size=3, padding=1)
    ])

    return SSD(num_classes, base_net_model, source_layer_indexes,
               extras, classification_headers, regression_headers,
               landmark_headers=landmark_headers,
               is_test=is_test, config=config, device=device)


def create_Mb_Tiny_RFB_fd_predictor(net, candidate_size=200, nms_method=None, sigma=0.5,
