...
class SimSPPF(nn.Module):
    """Simplified SPPF (YOLOv6 style): same pyramid pooling as SPPF but with ReLU activations.

    Drop-in replacement for SPPF (same args: c1, c2, k, n, shortcut). ReLU is cheaper than SiLU on
    edge hardware and quantizes better (INT8-friendly). Same parameter count as SPPF.
    """
    ...
    def __init__(self, c1: int, c2: int, k: int = 5, n: int = 3, shortcut: bool = False):
        ...
        self.cv2 = Conv(c_ * (n + 1), c2, 1, 1, act=nn.ReLU())
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.n = n
        self.add = shortcut and c1 == c2

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply sequential pooling with ReLU convs and return concatenated feature maps."""
        y = [self.cv1(x)]
        y.extend(self.m(y[-1]) for _ in range(getattr(self, "n", 3)))
        y = self.cv2(torch.cat(y, 1))
        return y + x if getattr(self, "add", False) else y
...
class RealNVP(nn.Module):
    ...
    @property
    def prior(self):
        """The prior distribution (validate_args off: a freak non-finite batch must not kill training)."""
        return torch.distributions.MultivariateNormal(self.loc, self.cov, validate_args=False)
...
class ChannelShuffle(nn.Module):
    """Channel shuffle operation for ShuffleNetV2."""
...
class ShuffleV2Block(nn.Module):
    ...
    For stride=1, c1 should equal c2.
    For stride=2, output channels are split across two branches.
    """

    def __init__(self, c1: int, c2: int, stride: int):
        super().__init__()
        ...
        if stride == 1:
            assert c1 == c2, "for stride=1, c1 must equal c2"
        ...
        c_branch = c1 // 2 if stride == 1 else c1

        self.branch2 = nn.Sequential(
            nn.Conv2d(c_branch, branch_features, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(branch_features),
            nn.SiLU(inplace=True),
...
class RepViTSE(nn.Module):
    """Squeeze-and-Excitation used in RepViT-style blocks."""

    def __init__(self, c: int, rd_ratio: float = 0.25):
        super().__init__()
        rd_channels = max(8, int(c * rd_ratio))
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
            nn.Conv2d(c, rd_channels, 1, bias=True),
            nn.SiLU(inplace=True),
            nn.Conv2d(rd_channels, c, 1, bias=True),
            nn.Sigmoid(),
        )
...
class RepViTBlock(nn.Module):
    """
    RepViT-style block for YOLO backbone replacement.

    Args:
        c1: input channels
        c2: output channels
        stride: 1 or 2
        hidden_ratio: expansion ratio for channel mixer
        use_se: whether to use SE after depthwise token mixer

    This block is intentionally simple for Ultralytics YAML integration:
    - stride=1: residual block if c1 == c2
    ...
    def __init__(
        ...
        hidden_dim = int(c2 * hidden_ratio)
        self.use_residual = stride == 1 and c1 == c2

        self.token_mixer = nn.Sequential(
            nn.Conv2d(c1, c1, kernel_size=3, stride=stride, padding=1, groups=c1, bias=False),
            nn.BatchNorm2d(c1),
            RepViTSE(c1) if use_se else nn.Identity(),
            nn.Conv2d(c1, c2, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(c2),
        )

        self.channel_mixer = nn.Sequential(
            nn.Conv2d(c2, hidden_dim, kernel_size=1, stride=1, padding=0, bias=False),
    ...
    def forward(self, x):
        x = x + self.token_mixer(x) if self.use_residual else self.token_mixer(x)
