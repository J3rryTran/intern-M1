    ...
    def load_image(
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im = imread(f, flags=self.cv2_flag)  # BGR
            if im is None:
                # Transient I/O failures (e.g. WSL2 9p mounts under parallel workers) make imread
                # return None for perfectly valid files — retry before declaring the image missing.
                for wait in (0.1, 0.5, 2.0):
                    time.sleep(wait)
                    im = imread(f, flags=self.cv2_flag)
                    if im is not None:
                        LOGGER.warning(f"{self.prefix}Transient read failure recovered after retry: {f}")
                        break
            if im is None:
