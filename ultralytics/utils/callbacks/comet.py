...
def _log_plots(experiment, trainer) -> None:
    ...
    """
    plot_filenames = None
    if type(trainer.validator.metrics).__name__ == "SegmentMetrics":
        plot_filenames = [
    ...
    elif isinstance(trainer.validator.metrics, DetMetrics) or type(trainer.validator.metrics).__name__ == "OBBMetrics":
        plot_filenames = [
            trainer.save_dir / f"{prefix}{plots}.png"
            for plots in EVALUATION_PLOT_NAMES
    ...
    if type(trainer.validator.metrics).__name__ != "ClassifyMetrics":
